  ...
  implements Storage<Item>
{
  ...
  private insertItem(item: Item, node: QuadNode<Item>, depth = 0) {
    ...
    if (
      ...
      // Move other items to children
      // for (const it of node.items) {
      //   if (containsRect(childRect, it.rect)) {
      //     this.removeItem(it)
      //     this.insertItem(it, node.children[childIndex]!, depth + 1)
      //   }
      // }
  ...
  private cleanupUpwards(node: QuadNode<Item>) {
    if (node.parent && node.items.size === 0 && isLeafNode(node)) {
      const index = this.getChildIndex(node.parent.rect, node.rect)
  ...
  *intersecting(rect: Rect): IterableIterator<Item> {
    ...
    while (stack.length) {
      ...
      for (const child of node.children) {
        if (child && intersects(child.rect, rect)) {
          stack.push(child)
        }
      }
    }
